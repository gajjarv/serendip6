...
def getFitsData(fileHandle):
	...
	return data, ccpwrs, hitPoints, ccpwrsPoints, nSteps, coarseID,NS,FS,CC


def plotPDF(fileHandle, data, hitPoints, ccpwrsPoints, ccpwrs, coarseID,NS,FS,CC):
	"""
	Prints plots to PDF
	"""
	...
	def powerHist():
		...
		# Plot histogram
		plt.figure()
		plt.hist(relpow, bins = 100) # arbitrary bin amount
		plt.axvline(lowthr, color='k',linestyle='--')
		plt.axvline(highthr, color='k',linestyle='--')
...
def main(filename):
	...
	data, ccpwrs, hitPoints, ccpwrsPoints, nSteps, coarseID,NS,FS,CC = getFitsData(fileHandle) # This opens fileHandle, will take a few minutes	
	...
	plotPDF(fileHandle, data, hitPoints, ccpwrsPoints, ccpwrs, coarseID,NS,FS,CC)
